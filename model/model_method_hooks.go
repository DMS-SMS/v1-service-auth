...
func (si *StudentInform) BeforeCreate(tx *gorm.DB) (err error) {
	...
	if query.RowsAffected != 0 {
		...
		return mysqlerr.DuplicateEntry(fmt.Sprintf("%d-%d-%02d", si.Grade.value, si.Class.value, si.StudentNumber.value), "number")
	}
...
func (ti *TeacherInform) BeforeCreate(tx *gorm.DB) (err error) {
	...
	if query.RowsAffected != 0 {
		...
		return mysqlerr.DuplicateEntry(fmt.Sprintf("%d-%d", ti.Grade.value, ti.Class.value), "class")
	}

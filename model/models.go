...
type StudentAuth struct {
	gorm.Model
	UUID       string `gorm:"PRIMARY_KEY;Type:char(20);UNIQUE;INDEX" validate:"uuid=student,len=20"` // 형식 => 'student-' + 12자리 랜덤 수 (20자)
	StudentId  string `gorm:"Type:varchar(20);NOT NULL;UNIQUE" validate:"min=4,max=20"`              // 4~20자 사이
...
type TeacherAuth struct {
	gorm.Model
	UUID      string `gorm:"PRIMARY_KEY;Type:char(20);UNIQUE;INDEX" validate:"uuid=teacher,len=20"` // 형식 => 'teacher-' + 12자리 랜덤 수 (20자)
	TeacherId string `gorm:"varchar(20);NOT NULL;UNIQUE" validate:"min=4,max=20"`                   // 4~20자 사이
...
type ParentAuth struct {
	gorm.Model
	UUID     string `gorm:"PRIMARY_KEY;Type:char(19);UNIQUE;INDEX" validate:"uuid=parent,len=19"` // 형식 => 'parent-' + 12자리 랜덤 수 (19자)
	ParentId string `gorm:"Type:varchar(20);NOT NULL;UNIQUE" validate:"min=4,max=20"`             // 4~20자 사이
...
type ParentInform struct {
	gorm.Model
	ParentUUID  string `gorm:"Type:char(19);UNIQUE;NOT NULL" validate:"uuid=parent,len=19"` // 형식 => 'parent-' + 12자리 랜덤 수
